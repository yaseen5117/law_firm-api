...
function uploadFile($request)
{
    if ($request->file_name) {
        if (
            Storage::exists($request->root_folder . '/' . $request->file_name)
        ) {
            Storage::move(
                $request->root_folder . '/' . $request->file_name,
                $request->root_folder .
                    '/' .
                    $request->attachmentable_id .
                    '/' .
                    $request->file_name
            );
        }
    ...
    info(
        'Helpers.php uploadFile Function: File mime_type' . $request->mime_type
    );

    if ($request->mime_type == 'application/pdf') {
        info(
            '****************CONVERTING PDF TO IMAGES START**********************'
        );
        ...
        try {
            ...
            for ($page = 0; $page < $num_page; $page++) {
                ...
                $generated_jpg_filename =
                    $page . ' - ' . $request->file_name . '.jpg';
    ...
    try {
        if ($request->hasFile($request->file_input_id)) {
            ...
            $fileName =
                md5(microtime()) .
                '.' .
                $orignalFile->getClientOriginalExtension();
            $orignalFile->storeAs(
                $request->root_directory . '/' . $request->id . '/',
                $fileName
            );
...
function to_date($date_in_any_format, $with_time = false)
{
    if (
        $date_in_any_format == '' ||
        $date_in_any_format == '0000-00-00' ||
        $date_in_any_format == '1969-12-31' ||
        $date_in_any_format == '1970-01-01'
    ) {
        return '';
    }
...
function initialism($str, $as_space = ['-'])
{
